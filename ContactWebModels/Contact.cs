...
namespace ContactWebModels
{
    public class Contact
    {
        ...
        [StringLength(ContactManagerConstants.MAX_EMAIL_EMAIL_LENGTH)]
        ...
        [RegularExpression("(^\\d{5}(-\\d{4})?$)|(^[ABCEGHJKLMNPRSTVXY]\\d{1}[A-Z]{1} *\\d{1}[A-Z]{1}\\d{1}$)", ErrorMessage= "Zip code is invalid.")]
        ...
        public string FriendlyAddress => State is null ? "" : string.IsNullOrWhiteSpace(StreetAddress1) ? $"{City}, {State.Abbrevation}, {Zip}" :
                                           string.IsNullOrWhiteSpace(StreetAddress2) ? $"{StreetAddress1}, {City}, {State.Abbrevation}, {Zip}" :
